
Recipe.create(
      name: "View More",     
      ingredients: '100g parmesan', 
      instruction: 'We envision a world where our private and public leaders prioritize the protection of our planet; where solutions to remedy climate change are equitable, lift up those that are most impacted, and are rooted in racial and economic justice; and where facts win out over fiction and there is universal agreement about the impacts of climate change on the earth and all of its inhabitants.',
      image:"data:image/png;base64,"
)
Recipe.create(
      name: "Peoples Climate Movement",     
      ingredients: 'https://peoplesclimate.org/, https://www.change.org/t/climate-action', 
      instruction: 'We envision a world where our private and public leaders prioritize the protection of our planet; where solutions to remedy climate change are equitable, lift up those that are most impacted, and are rooted in racial and economic justice; and where facts win out over fiction and there is universal agreement about the impacts of climate change on the earth and all of its inhabitants.',
      image: "https://media11.s-nbcnews.com/j/MSNBC/Components/Video/201909/f_mo_greta_thunberg_un_3_190923.focal-760x428.jpg"
)
Recipe.create(
      name: "March For Our Lives",
      ingredients: 'https://marchforourlives.com/, https://www.change.org/p/march-for-our-lives-an-act-to-protect-save-your-children',
      instruction: 'To harness the power of young people across the country to fight for sensible gun violence prevention policies that save lives.',
      image: "https://ichef.bbci.co.uk/news/976/cpsprodpb/81E7/production/_100555233_mediaitem100555032.jpg"
)
Recipe.create(
      name: "United We Dream",
      ingredients: 'https://unitedwedream.org/, https://www.change.org/p/u-s-senate-protecting-dreamers?recruiter=813423694&utm_source=share_petition&utm_medium=twitter&recruited_by_id=7769dff0-b24e-11e7-91fc-47a31525608c',
      instruction: 'When you’re undocumented, you face a lot of discrimination, and that creates a lot of fear. At United We Dream, we transform that fear into finding your voice. We empower people to develop their leadership, their organizing skills, and to develop our own campaigns to fight for justice and dignity for immigrants and all people. This is achieved through immigrant youth-led campaigns at the local, state, and federal level.',
      image: "https://lh3.googleusercontent.com/proxy/bSwfEOw_MhHWZbFHMLGkLl7B5Rw_5-vtLFmeAi-R_4ki4wGDCa2wpvnRHoK8rOs_jOjQddBactQFxRx0ES623JWT3qPcnYMTMOAKn-HIoae8rF8_eSKhtMEh_Z9s2sLSXMqpk0239r7fRf8t"
)
Recipe.create(
      name: "Voting Rights",
      ingredients: 'https://www.aclu.org/know-your-rights/voting-rights/, https://www.change.org/t/voting-en-us',
      instruction: 'Voting is the cornerstone of our democracy and the fundamental right upon which all our civil liberties rest. The ACLU works to protect and expand Americansʼ freedom to vote. In addition to this litigation, we are working with our affiliates to advocate for policies that make it easier for Americans to vote, such as the expansion of same-day and online voter registration. ',
      image: "https://www.aclu-ms.org/sites/default/files/styles/featured_image_mobile_480x319/public/field_image/issue-voting-rights.jpg?itok=jFyko93x"
)
Recipe.create(
      name: "The Fight for $15",
      ingredients: 'https://fightfor15.org/, https://www.change.org/t/fight-for-15-en-us',
      instruction: 'It’s time to pay people enough to survive. It’s time to pay people what they deserve.  It’s time corporations like McDondald’s give us the workplace protections we deserve.It’s time for $15/hr and #UnionsForAll.',
      image: "https://beyondchron.org/wp-content/uploads/2019/08/3_Page_01-3.jpg"
)
Recipe.create(
      name: "My Body My Rights",
      ingredients: 'https://www.amnesty.org/en/get-involved/my-body-my-rights/, https://www.change.org/p/georgia-state-house-women-s-rights-my-body-my-choice',
      instruction: "Whoever you are, wherever you live, you have the right to make these choices without fear, violence or discrimination. Yet all over the world, people are bullied, discriminated against and arrested, simply for making choices about their bodies and their lives. Join us in defending sexual and reproductive rights for all. It's your body. Know Your rights",
      image: "https://assets.change.org/photos/9/ub/bp/bGubBpmLsnPwHqW-1600x900-noPad.jpg?1557947075"
)
Recipe.create(
      name: "Me Too",
      ingredients: 'https://metoomvmt.org/, https://www.change.org/m/metoo',
      instruction: 'The ‘me too.’ Movement believes in the radical possibilities of a movement against sexual violence, led by survivors of sexual violence.',
      image: "https://econreview.berkeley.edu/wp-content/uploads/2018/11/2930_3832_97.jpg"

)
Recipe.create(
      name: "Black Lives Matter",
      ingredients: 'https://blacklivesmatter.com/, https://www.change.org/t/black-lives-matter-en-us',
      instruction: 'Four years ago, what is now known as the Black Lives Matter Global Network began to organize. It started out as a chapter-based, member-led organization whose mission was to build local power and to intervene when violence was inflicted on Black communities by the state and vigilantes. In the years since, we’ve committed to struggling together and to imagining and creating a world free of anti-Blackness, where every Black person has the social, economic, and political power to thrive.',
      image:"https://img2.chinadaily.com.cn/images/202006/16/5ee81a1ca3108348fcd5b61b.jpeg"
)


